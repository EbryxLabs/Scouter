...
def azure_vault_logging_enabled():
    
  current_script_name = "azure_vault_logging_enabled"
  ...
  for subscription in subscriptions:
      ...
      for item in data.get("value", []):
          ...
          diagnostic_settings_endpoint = f"https://management.azure.com/{vault_id}/providers/Microsoft.Insights/diagnosticSettings?api-version=2021-05-01-preview"
          headers = {"Authorization": f"Bearer {credentials.get_token('https://management.azure.com').token}"}
          response = requests.get(diagnostic_settings_endpoint, headers=headers)
          data = response.json()
          print(data)
