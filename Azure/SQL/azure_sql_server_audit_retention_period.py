...
def azure_sql_server_audit_retention_period():
  current_script_name = "azure_sql_server_audit_retention_period"
  ...
  for subscription in subscriptions:
      ...
      for group in list(group_list):
          ...
          for item in data.get("value", []):
              ...
              diagnostic_settings_endpoint = f"https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Sql/servers/{server_name}/auditingSettings?api-version=2021-11-01"
              headers = {"Authorization": f"Bearer {credentials.get_token('https://management.azure.com').token}"}
              response = requests.get(diagnostic_settings_endpoint, headers=headers)
