...
def azure_sql_db_advanced_threat_protection_enabled():
  current_script_name = "azure_sql_db_advanced_threat_protection_enabled"
      ...
      for group in list(group_list):
          ...
          for item in data.get("value", []):
              ...
              for item in dbs.get("value",[]):
                ...
                db_id=item.get("id",{})
                diagnostic_settings_endpoint = f"https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Sql/servers/{server_name}/databases/{db_name}/advancedThreatProtectionSettings?api-version=2021-11-01"
                headers = {"Authorization": f"Bearer {credentials.get_token('https://management.azure.com').token}"}
                response = requests.get(diagnostic_settings_endpoint, headers=headers)
                ...
                for item in db_info.get("value",[]):
                  ...
                  result_list.append({"db_id": db_id, "policy_name":name, "state":state})
